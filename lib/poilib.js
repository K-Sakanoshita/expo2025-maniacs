    ...
    setPoi(target, actonly, flist) {
        ...
        const checkMarker = function (poi, LL, zoomlv) {
            ...
            if (geoCont.checkInner(poi.lnglat, LL)) {
                ...
                if (viewflag && tgv) params = { target: target, poi: poi, actlists: actlists }  // act ok and 未表示の場合
            }
            return params
        }

        // params内のgeojsonをgeojson変数に保存
        const saveMarker = function (params) {
            ...
            params.poi.geojson.properties.name = name
        ...
        markers.forEach(marker => {
            if (!mapLibre.map.getLayer('marker-bg-' + marker)) {
                mapLibre.map.addLayer({
                    ...
                    layout: {
                        ...
                        'icon-image': 'circle_' + marker + '.png', 'icon-size': Conf.icon[marker]
                    }
                })
            }
            if (!mapLibre.map.getLayer('marker-fg-' + marker)) {
                ...
                mapLibre.map.on('mouseenter', 'marker-fg-' + marker, () => {    // ポインター変更（カーソルを手に）
                    mapLibre.map.getCanvas().style.cursor = 'pointer';
                });
                mapLibre.map.on('mouseleave', 'marker-fg-' + marker, () => {

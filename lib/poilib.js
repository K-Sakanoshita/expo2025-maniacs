...
class PoiCont {
    ...
    setPoi(flist, actonly) {
        ...
        const saveMarker = function (params) {              // params内のgeojsonをgeojson変数に保存
            ...
            name += (visited[1] !== "" && visited[1] !== undefined) ? `\n(${visited[1]})` : "";
            ...
            mapLibre.map.addLayer({
                ...
                layout: {
                    ...
                    'icon-image': ['case', ['all', ['has', 'level'], ['>=', ['to-number', ['get', 'level']], 1]],
                        ...
                        [
                            'step', ['zoom'], Conf.icon["shadow"] * 0.7,    // ズーム13以下は 0.6 倍
                            16, Conf.icon["shadow"] * 0.9,      // ズーム15以下は 0.8 倍
                            17, Conf.icon["shadow"] * 1.1       // ズーム16以上は 1.0 倍
                        ]
                ...
                mapLibre.map.addLayer({
                    ...
                    layout: {
                        ...
                        'icon-image': 'circle_' + marker + '.png', 'icon-size':
                            [
                                'step', ['zoom'], Conf.icon[marker] * 0.7,    // ズーム13以下は 0.6 倍
                                16, Conf.icon[marker] * 0.9,    // ズーム15以下は 0.8 倍
                                17, Conf.icon[marker] * 1.1     // ズーム16以上は 1.0 倍
                            ]
                    }
                })
            }
                    ...
                    layout: {
                        ...
                        'icon-image': this.#layerMatch, 'icon-size':
                            [
                                ...
                                16, Conf.icon[marker] * 0.5,    // ズーム15以下は 0.5 倍
                                17, Conf.icon[marker] * 0.7     // ズーム16以上は 0.7 倍
                            ]
                    }
                })
            ...
            if (!mapLibre.map.getLayer('marker-text-' + marker)) {
                mapLibre.map.addLayer({
                    ...
                    layout: {
                        ...
                        'text-justify': 'auto', 'text-padding': 1,

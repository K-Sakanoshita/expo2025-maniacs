...
class PoiCont {
    ...
    setPoi(flist, actonly) {
        ...
        const saveMarker = function (params) {              // params内のgeojsonをgeojson変数に保存
            ...
            name += (poiStatus[PoiStatusIndex.MEMO] !== undefined && poiStatus[PoiStatusIndex.MEMO] !== "")? `\n(${poiStatus[PoiStatusIndex.MEMO]})` : ""; // memoがあれば追加
            ...
            if (!mapLibre.map.getLayer('marker-fg-' + marker)) {
                mapLibre.map.addLayer({
                    ...
                    layout: {
                        ...
                        "text-anchor": "left", "text-offset": [Conf.icon[marker] * 2.8, 0],  // テキスト位置、アイコンのオフセット

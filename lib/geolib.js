...
class Maplibre {
    ...
    init(Conf) {
        const extractFilenames = function (obj, result = new Set()) {
            for (const key in obj) {
                const value = obj[key];
                if (typeof value === 'string' && value.endsWith('.svg')) {
                    result.add(value);
                } else if (typeof value === 'object' && value !== null) {
                    extractFilenames(value, result);
                }
            }
            return result;
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            const fnames1 = extractFilenames(Conf.marker.tag)
            ...
            this.map.on('load', async () => {
                ...
                for (let file of fnames2) {
                    let image = await this.map.loadImage("./" + Conf.icon.fgPath + "/" + file)
                    this.map.addImage(file, image.data)
                }

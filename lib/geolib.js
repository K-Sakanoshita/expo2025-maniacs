...
class Maplibre {
    ...
    // ミニマップ表示 params {center: [],zoom:}
    addMiniMap(params) {
        return new Promise((resolve) => {
            const mmap = document.getElementById("mini-map")
            if (this.minimap === null || this.minimap === undefined) {
                this.minimap = new maplibregl.Map({
                    container: 'mini-map', style: 'https://demotiles.maplibre.org/style.json',
                ...
                this.minimap.on('style.load', () => {
                    ...
                    this.minimap.addLayer({ id: 'alls-line', type: 'line', source: 'alls', paint: { 'line-color': '#AA3333', 'line-width': 1 } })
                    this.minimap.addLayer({ id: 'sels-line', type: 'line', source: 'sels', paint: { 'line-color': '#FFFF66', 'line-width': 2 } })
                    this.minimap.setProjection({ "type": "globe" }) // globe表示には fog が必要
                    mmap.classList.remove("d-none")
                    this.minimap.flyTo(params)
    ...
    #miniMapClick(e) {
        ...
        if (matches.length > 0) {               // 地図移動（miniMapとmapの両方）
            ...
            if (OSMID !== "") {
                if (cMapMaker.minimap) winCont.closeModal()
    ...
    showCountryByCode(CCode) {
        ...
        if (match.length > 0) this.#highlightCountry(match);
        else console.warn("showCountryByCode: Not found: ", countries[0]);
    }
    ...
    #highlightCountry(feature) {
        ...
        this.addMiniMap({ center: Conf.map.viewCenter, zoom: Conf.map.initZoom / 4 }).then(() => highlight())
    }
...
class GeoCont {
    ...
    flashPolygon(geojson) {        // 数秒間だけ指定したポリゴンを描写
        ...
        const processedFeature = (geojson.geometry.type === "Point")
            ? turf.circle(geojson.geometry.coordinates, 6, { steps: 64, units: "meters" }) : geojson;
        ...
        if (source == undefined) {
            ...
            this.#flashing = setTimeout(() => { fadeOutLayer(mapLibre.map, layerId, sourceId, 1500, 30) }, 4000);    // 4秒後にフェードアウト

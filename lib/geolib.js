...
class GeoCont {
    ...
    flashPolygon(geojson) {        // 3秒間だけ指定したポリゴンを描写
        ...
        if (source == undefined) {
            ...
            setTimeout(() => { fadeOutLayer(mapLibre.map, layerId, sourceId, 1000, 20) }, 3000);        // 3秒後にフェードアウト
            function fadeOutLayer(map, layerId, sourceId, duration = 1000, steps = 10) {        // フェードアウト関数

...
class VisitedCont {
    ...
    getValueByOSMID(osmid) {
        ...
        let visited = lcal == null ? ["false", "false", ""] : lcal.split(",")
        visited[0] = visited[0].toLowerCase() === "true"        // true時はboolean型のtrueを返し、他はfalse
        visited[1] = visited[1] === undefined ? "false" : visited[1].toLowerCase() === "true"; // お気に入りフラグはundefined時はfalseを返す

        let returnValue = visited;
        if (visited.length == 1) {
            returnValue = [visited[0], false, ""]
        }
        else if (visited.length == 2) {
            // お気に入りフラグ
            visited[1] = visited[1].toLowerCase() === "true"
            returnValue = [visited[0], visited[1], ""]
        }
        return returnValue;
    }
    ...
    import_load() {
        ...
        if (csvInput.files.length > 0) {    // ファイルが選択された時
            ...
            reader.onload = function (e) {
                ...
                if (!header.startsWith("key,category,name,visited,favorite,memo")) {
                ...
                lines.forEach(line => {
                    ...
                    const key = values[0];
                    const visited = values[3].toLowerCase() + "," + values[4].toLowerCase() + "," + values[5]; // visited列の値だけを使う
